 ...
 */
package org.front.end.wits.vaadin.data;
...
public class LazyLoadingContainer implements Container.Indexed, Container.Sortable{
	...
	public int getIndexForObjectIdTask(String id) {
		Item searched = lazyLoadingQuery.loadSingleResult((String) id); 
		if(searched == null) {
			return -1;
		}
		return getIndexForObjectIdTask(searched, 0, size()-1);
	}

	@SuppressWarnings({"unchecked", "rawtypes"})
	public int getIndexForObjectIdTask(Item searched, int low, int high) {
		if (high < low) {
			return -1; // not found
		}
		...
		if (!(searched instanceof Comparable)
			...
			throw new ActivitiException("Cannot use the getIndexForObjectIdTask method for non-Comparables");
		}

		int comparison = ((Comparable) searched).compareTo((Comparable) result); 
		if (comparison != 0) {
			return getIndexForObjectIdTask(searched, middle+1, high);
		} else {
			return middle;
		}
	}
