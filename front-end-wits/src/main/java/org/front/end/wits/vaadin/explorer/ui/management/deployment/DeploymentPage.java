...
import org.front.end.wits.vaadin.navigation.DeploymentNavigator;
...
public class DeploymentPage extends ManagementPage {
	...
	public DeploymentPage() {
		...
		deploymentFilter = WitsApp.get().getComponentFactory(DeploymentFilterFactory.class).create();
	}
	...
	protected void initUi() {
		...
		if (deploymentId == null) {
			...
			selectElementFirstTable(deploymentListContainer.getIndexForObjectId(deploymentId));
		}
	}
		...
		deploymentTable.addListener(new Property.ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				if(item != null) {
					...
					WitsApp.get().setCurrentUriFragment(new UriFragment(DeploymentNavigator.DEPLOYMENT_URI_PART));
				}
			}
		});
