...
import org.front.end.wits.vaadin.ui.custom.DetailPanel;
import org.front.end.wits.vaadin.ui.form.FormPropertiesForm;
import org.front.end.wits.vaadin.ui.mainlayout.ExplorerLayout;
import org.front.end.wits.vaadin.ui.view.workflow.process.listener.EditModelClickListener;
...
public class EditorProcessDefinitionDetailPanel extends DetailPanel {
    ...
    actionSelect.addListener(new ValueChangeListener() {
      ...
      public void valueChange(ValueChangeEvent event) {
        if (i18nManager.getMessage(Messages.PROCESS_COPY).equals(event.getProperty().getValue())) {
          WitsApp.get().getViewManager().showPopupWindow(new CopyModelPopupWindow(modelData));
        } else if (i18nManager.getMessage(Messages.PROCESS_DELETE).equals(event.getProperty().getValue())) {
          WitsApp.get().getViewManager().showPopupWindow(new DeleteModelPopupWindow(modelData));
  ...
  protected void exportModel() {
    final FileResource stream = new FileResource(new File(""), WitsApp.get()) {
      ...
      private static final long serialVersionUID = 1L;
        ...
        public DownloadStream getStream() {
          ...
          try {
            ...
            WitsApp.get().getNotificationManager().showErrorNotification(Messages.PROCESS_TOXML_FAILED, e);
          }
          return ds;
        }
    };
    stream.setCacheTime(0);
    WitsApp.get().getMainWindow().open(stream);
  }
  ...
  protected void deployModel() {
    try {
      ...
      WitsApp.get().getViewManager().showDeploymentPage(deployment.getId());
