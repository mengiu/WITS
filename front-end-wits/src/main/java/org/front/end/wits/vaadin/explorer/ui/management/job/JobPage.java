...
import org.front.end.wits.vaadin.navigation.JobNavigator;
...
public class JobPage extends ManagementPage {
	...
	protected void initUi() {
		...
		if (jobId == null) {
			...
			selectElementFirstTable(jobListContainer.getIndexForObjectId(jobId));
		}
	}
		...
		jobTable.addListener(new Property.ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				if(item != null) {
					...
					WitsApp.get().setCurrentUriFragment(new UriFragment(JobNavigator.JOB_URL_PART));
				}
			}
		});
	...
	public void refreshCurrentJobDetails() {
		if(firstTable.getValue() != null) {
			Item selectedJob = firstTable.getItem(firstTable.getValue());
